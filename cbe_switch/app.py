...
def preview_profile(profile_id: str, payload: dict[str, Any] | None = None) -> dict[str, str]:
    try:
        ...
        config, auth = store.render(store._to_template(profile["config_template"], profile["key"], profile["url"]), store._to_auth_template(profile["auth_template"], profile["key"]), profile["key"], profile["url"])
