...
def preview_profile(profile_id: str, payload: dict[str, Any] | None = None) -> dict[str, str]:
    try:
        ...
        config, auth = store.render(store._to_template(profile["config_template"], profile["key"], profile["url"]), store._to_auth_template(profile["auth_template"], profile["key"]), profile["key"], profile["url"], profile.get("permission_mode", "ask"))
...
def backup(backup_id: str) -> dict[str, Any]:
    try:
        return store.get_backup(backup_id)
    except Exception as exc:
        raise error(exc) from exc
